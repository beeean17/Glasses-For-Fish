...
class ImageSampling:
    ...
    def get_default_config():
        ...
        # Lowering the default threshold makes frame selection less aggressive,
        # which is friendlier for videos with smoother camera motion.
        config['distance_threshold'] = 0.05
    ...
    def evaluate_frame(self, frame, all_descriptors):
        blur_score = self.laplacian_blur_metric(frame)
        if blur_score <= self.config['blur_threshold']:
            return False, None, f"Skipped: blur score {blur_score:.2f} <= {self.config['blur_threshold']}"

        try:
            new_frame_descriptor = self.multi_resolution_descriptor(frame, cell_size=self.config['cell_size'])
        except ValueError:
            return False, None, (
                "Skipped: chessboard corners not found. "
                f"Check chessboard_pattern (current: {self.config['chessboard_pattern']})."
            )

        if len(all_descriptors) == 0:
            return True, new_frame_descriptor, "Selected first valid frame."

        last_selected_descriptor = all_descriptors[-1]
        last_distance = round(hamming(new_frame_descriptor, last_selected_descriptor), 2)
        if last_distance <= self.config['distance_threshold']:
            return False, None, (
                "Skipped: frame is too similar to the last selected frame. "
                f"distance {last_distance} <= {self.config['distance_threshold']}. "
                "Lower distance_threshold in config if you want denser sampling."
            )

        new_frame_multi_descriptors = [
            self.multi_resolution_descriptor(frame, resolution_level=resolution_level)
            for resolution_level in self.config['resolution_levels']
        ]
        multi_resolution_score_total = sum(
            self.multi_resolution_score(descriptor, resolution_level)
            for descriptor, resolution_level in zip(new_frame_multi_descriptors, self.config['resolution_levels'])
        )
        if multi_resolution_score_total <= self.config['multi_score_threshold']:
            return False, None, (
                "Skipped: multi-resolution score too low. "
                f"{multi_resolution_score_total} <= {self.config['multi_score_threshold']}"
            )

        return True, new_frame_descriptor, "Selected frame."

    def save_selected_frame(self, frame, selected_image_count, selected_images, all_descriptors):
        is_selected, descriptor, message = self.evaluate_frame(frame, all_descriptors)
        print(message)
        if not is_selected:
            return selected_image_count, message
        ...
        selected_images.append(frame.copy())
        all_descriptors.append(descriptor)
        saved_message = f"Saved: {image_save_path}"
        print(saved_message)
        return selected_image_count, saved_message
    ...
    def run_img_sampling(self):
        ...
        paused = False
        frame = None
        status_message = 'Space: pause/resume | Enter: select | Esc: exit'
        ...
        while True:
            ...
            cv.putText(display_frame, f'Number of Selected Images: {selected_image_count}', self.config['status_offset'], self.config['status_font'], self.config['status_font_scale'], self.config['text_color'])
            cv.putText(display_frame, 'Space: pause/resume | Enter: select | Esc: exit', (10, 50), self.config['status_font'], self.config['status_font_scale'], self.config['text_color'])
            cv.putText(display_frame, status_message, (10, 75), self.config['status_font'], self.config['status_font_scale'], self.config['text_color'])
            ...
            if key in (ord('\r'), ord('\n')):
                selected_image_count, status_message = self.save_selected_frame(frame, selected_image_count, selected_images, all_descriptors)
                paused = False
                continue
...
if __name__ == '__main__':
    ...
    img_selection = ImageSampling(args.video_path, args.out_dir, args.config_file)
